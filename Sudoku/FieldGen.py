...
def faster_mask_random_cells(field: NDArray[np.uint8], diff: int, HIDE_CELLS: list[int] = [26, 38, 52]):
    coords: list[tuple[int, int]] = [(i, k) for i in range(9) for k in range(9)]
    random.shuffle(coords)
    counter = 0
    for x, y in coords:
        if counter >= HIDE_CELLS[diff]:
            return True
        if field[x, y] != 0:
            temp = field[x, y]
            field[x,y] = 0
            if has_unique_solutions(deepcopy(field)):
                counter += 1
            else:
                field[x, y] = temp
    return False
...
def create_field_and_mask(difficult: int) -> tuple[NDArray[np.uint8], NDArray[np.bool_]]:
    ...
    while True:
        ...
        if faster_mask_random_cells(field_with_masked_cells, difficult):
